...
module.exports.run = async (bot,message,args) => {
    ...
    //if(message.guild.id != '445412739607691265' || message.guild.id != '423211874855682059') return
    ...
    if(!player.roles.has(gRole.id)) return message.channel.send("They don't that Role.");
    await(player.removeRole(gRole.id));
    
    message.channel.send(`Rainbow role is removed for <@${player.id}>`);
}
